(function () {
  ...
  Game.prototype.normalizeWeaponShopItems = function (data) {
    var weaponIds = ["bomb", "mortar", "redBullet"];
    var ownedId = weaponIds.find(function (id) { return data.items[id] > 0; });
    weaponIds.forEach(function (id) {
      if (id !== ownedId) { data.items[id] = 0; }
    });
  };
  ...
  Game.prototype.purchaseShopItem = function (category, id) {
    ...
    if (["boosts", "items"].indexOf(category) !== -1 && !this.isShopItemAvailable(category, id)) { return { ok: false, reason: "mode" }; }
    if (category === "items" && ["bomb", "mortar", "redBullet"].indexOf(id) !== -1 &&
      ["bomb", "mortar", "redBullet"].some(function (weaponId) {
        return weaponId !== id && this.shopData.items[weaponId] > 0;
      }, this)) {
  ...
  Game.prototype.openShop = function () {
    if (this.state !== Config.states.MENU) { return; }
    this.setState(Config.states.SHOP);
    if (TankGame.Audio.setMusicTrack) { TankGame.Audio.setMusicTrack(Config.shopBackgroundMusic, false); }
  };

  Game.prototype.closeShop = function () {
    if (this.state !== Config.states.SHOP) { return; }
  ...
  Game.prototype.activateRunShop = function () {
    ...
    if (this.selectedMode === "endless") {
      this.runShop.mudTruck = this.shopData.items.mudTruck || 0;
      this.shopData.items.mudTruck = 0;
    }
    ["bomb", "mortar", "redBullet"].forEach(function (id) {
      this.runShop[id] = this.shopData.items[id] || 0;
      this.shopData.items[id] = 0;
    }, this);
    this.shopHealingTimer = 10;
    this.saveShopData();
  };
  ...
  Game.prototype.createRewardOptions = function (pool, count) {
    var available = pool.filter(function (reward) {
      ...
      if (reward.id === "frontStep" && this.runShop.mudTruck) { return false; }
  ...
  Game.prototype.resetWorld = function (preserveRun) {
    ...
    if (this.selectedMode === "endless" && this.runShop.mudTruck) {
      this.player.moveSpeedMultiplier *= 1.1;
      this.player.visualScale = 1.1;
      this.player.radius = 23 * 1.1;
    }
    this.player.baseVisualScale = this.player.visualScale || 1;
    this.player.baseRadius = this.player.radius || 23;
    if (this.runShop.frenzy) {
  ...
  Game.prototype.updateLevelMusic = function () {
    ...
    var track = bossMusicActive ? Config.bossBackgroundMusic :
      (this.selectedMode === "challenge" ? Config.challengeBackgroundMusic : Config.backgroundMusic);
    if (TankGame.Audio.setMusicTrack) {
      TankGame.Audio.setMusicTrack(track, false);
    }
  };
  ...
  Game.prototype.update = function (deltaTime) {
    ...
    if (this.runShop.healing && this.player.alive) {
      ...
      if (this.shopHealingTimer <= 0) {
        this.shopHealingTimer += 10;
        this.player.health = Math.min(this.player.maxHealth, this.player.health + 100 * this.runShop.healing);
      }
    }
  ...
  Game.prototype.damageEnemy = function (enemy, damage, effectX, effectY) {
    ...
    enemy.wreck = true;
    enemy.wreckLife = 3;
    enemy.wreckParticles = this.createWreckParticles(enemy);
  ...
  Game.prototype.createWreckParticles = function (enemy) {
    var scale = enemy.visualScale || 1;
    return Array.from({ length: 84 }, function () {
      return {
        x: (Math.random() - 0.5) * 64 * scale,
        y: (Math.random() - 0.5) * 58 * scale,
        rise: (12 + Math.random() * 27) * scale,
        sway: (5 + Math.random() * 12) * scale,
  ...
  Game.prototype.fire = function (tank) {
    ...
    var isPlayerSpecialMode = tank.team === "player" && ["endless", "brave"].indexOf(this.selectedMode) !== -1;
    ...
    var mortarTarget = trackingActive ? this.markedTarget : null;
    var mortarTargetX = mortarTarget ? mortarTarget.x : this.input.pointer.x;
    var mortarTargetY = mortarTarget ? mortarTarget.y : this.input.pointer.y;
    var playerShotVariant = null;
    if (isPlayerSpecialMode && this.runShop.bomb && Math.random() < Math.min(1, this.runShop.bomb * 0.1)) {
      playerShotVariant = "bomb";
    } else if (isPlayerSpecialMode && this.runShop.mortar && Math.random() < Math.min(1, this.runShop.mortar * 0.01)) {
      playerShotVariant = "mortar";
    }
    var self = this;
    for (var bulletIndex = 0; bulletIndex < bulletCount; bulletIndex += 1) {
      ...
      if (playerShotVariant === "bomb") {
        ...
        bullet.explosionRadius = bulletCount > 1 ? 56 : 80;
        bullet.playerBombRadius = bullet.explosionRadius;
      } else if (playerShotVariant === "mortar") {
        bullet.playerMortar = true;
        bullet.mortarProjectile = true;
        bullet.mortar = true;
        bullet.bossBomb = true;
        bullet.radius = 9;
        bullet.startX = x;
        bullet.startY = y;
        var targetOffsetRadius = splitLevel ? 40 + Math.random() * 140 : 0;
        var targetOffsetAngle = Math.random() * Math.PI * 2;
        bullet.targetOffsetX = Math.cos(targetOffsetAngle) * targetOffsetRadius;
        bullet.targetOffsetY = Math.sin(targetOffsetAngle) * targetOffsetRadius;
        bullet.targetX = mortarTargetX + bullet.targetOffsetX;
        bullet.targetY = mortarTargetY + bullet.targetOffsetY;
        bullet.trackingTarget = mortarTarget;
  ...
  Game.prototype.movePlayerSafely = function (tank, angle, distance, startX, startY) {
    ...
    if (tank.team === "player" && this.runShop.mudTruck) {
      this.destroyBreakableWallsAlongPath(pathStart, { x: nextX, y: nextY }, tank.radius, true);
    }
    ...
    if (tank.team === "player" && this.runShop.mudTruck && Math.hypot(pathEnd.x - pathStart.x, pathEnd.y - pathStart.y) > 1) {
      this.damageEnemiesAlongPath(pathStart, pathEnd, (tank.bulletDamage || Config.bulletDamage) * 2);
    }
  ...
  Game.prototype.destroyBreakableWallsInRadius = function (x, y, radius, awardScore) {
    var self = this;
    if (!this.worldMap || !this.worldMap.obstacles) { return 0; }
    var circle = { x: x, y: y, radius: Math.max(0, radius || 0) };
    var walls = this.worldMap.obstacles.slice().filter(function (obstacle) {
      return obstacle.kind === "B" && TankGame.Collision.circleIntersectsRectangle(circle, obstacle);
    });
    walls.forEach(function (wall) {
      if (self.onFieldWallBroken) { self.onFieldWallBroken(wall); }
      TankGame.Map.removeObstacle(self.worldMap, wall);
      if (awardScore) { self.score += 10; }
    });
    return walls.length;
  };

  Game.prototype.destroyBreakableWallsAlongPath = function (start, end, padding, awardScore) {
    var self = this;
    if (!this.worldMap || !this.worldMap.obstacles) { return 0; }
    var walls = this.worldMap.obstacles.slice().filter(function (obstacle) {
      return obstacle.kind === "B" && TankGame.Collision.segmentIntersectsRectangle(
        start.x, start.y, end.x, end.y, obstacle, padding || 0
      );
    });
    walls.forEach(function (wall) {
      if (self.onFieldWallBroken) { self.onFieldWallBroken(wall); }
  ...
  Game.prototype.updateBullet = function (bullet, deltaTime) {
    ...
    if (bullet.fixedTurretMortar || bullet.mortarProjectile) {
      if (bullet.playerMortar && bullet.trackingTarget && bullet.trackingTarget.alive) {
        bullet.targetX = bullet.trackingTarget.x + (bullet.targetOffsetX || 0);
        bullet.targetY = bullet.trackingTarget.y + (bullet.targetOffsetY || 0);
      }
    ...
    if (wall) {
      ...
      if (!canPierceWall && bullet.team === "player" && this.endlessPermanent.explosive && !bullet.fragment && !bullet.bossBomb) { this.spawnFragments(bullet.x, bullet.y, bullet.damage); }
      if (wall.kind === "B" && bullet.team === "player" && (bullet.bossBomb || bullet.redBullet)) {
        if (bullet.redBullet) { this.destroyBreakableWallsInRadius(bullet.x, bullet.y, bullet.radius + 1, true); }
      } else if (wall.kind === "B") {
    ...
    if (bullet.team === "player" && this.runShop.instantKill && !target.isBoss && !target.isElite && !target.isBossClone &&
    ...
    if (bullet.team === "player" && this.endlessPermanent.explosive && !bullet.fragment) { this.spawnFragments(bullet.x, bullet.y, bullet.damage); }
  ...
  Game.prototype.detonatePlayerBomb = function (bullet) {
    ...
    if (bullet.bossBombDetonated) { return false; }
    bullet.bossBombDetonated = true;
    if (bullet.mortarFragment) {
      this.enemies.forEach(function (enemy) {
        if (!enemy.alive || Math.hypot(enemy.x - bullet.x, enemy.y - bullet.y) > enemy.radius + bullet.radius) { return; }
        self.damageEnemy(enemy, bullet.damage, bullet.x, bullet.y);
      });
      this.destroyBreakableWallsInRadius(bullet.x, bullet.y, bullet.radius, true);
      TankGame.Effects.burst(bullet.x, bullet.y, "#ffd166", 12, 140);
      return true;
    }
    ...
    this.destroyBreakableWallsInRadius(bullet.x, bullet.y, radius, true);
    if (!bullet.mortarFragment && this.endlessPermanent.explosive) {
      if (bullet.playerMortar) {
        this.spawnMortarFragments(bullet.x, bullet.y, bullet.damage);
      } else if (bullet.playerBomb && !bullet.fragment) {
        this.spawnBombFragments(bullet.x, bullet.y, bullet.damage);
      } else if (!bullet.fragment) {
        this.spawnFragments(bullet.x, bullet.y, bullet.damage);
      }
    }
  ...
  Game.prototype.spawnMortarFragments = function (x, y, damage) {
    var multiplier = this.getExplosiveDamageMultiplier();
    for (var i = 0; i < 8; i += 1) {
      var angle = Math.PI * 2 * i / 8;
      var fragment = TankGame.Entities.createBullet(x, y, angle, "player");
      fragment.damage = damage * multiplier;
      fragment.speed = Config.bulletSpeed * 0.72;
      fragment.radius = 6;
      fragment.lifetime = 0.48;
      fragment.armingTime = 0;
      fragment.fragment = true;
      fragment.mortarProjectile = true;
      ...
      fragment.startX = x;
      fragment.startY = y;
      fragment.targetX = x + Math.cos(angle) * 200;
      fragment.targetY = y + Math.sin(angle) * 200;
      fragment.flightDuration = 0.48;
      fragment.flightTimer = fragment.flightDuration;
      this.bullets.push(fragment);
    }
    TankGame.Effects.burst(x, y, "#ffd166", 10, 180);
  };

  Game.prototype.spawnBombFragments = function (x, y, damage) {
    var multiplier = this.getExplosiveDamageMultiplier();
    for (var i = 0; i < 8; i += 1) {
      var angle = Math.PI * 2 * i / 8 + (Math.random() - 0.5) * 0.35;
      var fragment = TankGame.Entities.createBullet(x, y, angle, "player");
      fragment.damage = damage * multiplier;
      fragment.speed = Config.bulletSpeed * 0.68;
      fragment.radius = 7;
      fragment.lifetime = Config.bulletLifetime * 0.6;
      fragment.armingTime = 0.1;
      fragment.fragment = true;
