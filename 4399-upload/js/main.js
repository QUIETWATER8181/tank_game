(function () {
  ...
  function syncInterface() {
    if (window.TankGame4399) {
      window.TankGame4399.progress(game.state === Config.states.MENU ? 25 : 100);
      if (game.state === Config.states.VICTORY || game.state === Config.states.DEFEAT) {
        window.TankGame4399.submitScore(game.score);
      }
    }
  ...
  shareButton.addEventListener("click", function () {
    if (window.TankGame4399) { window.TankGame4399.share(); }
  });
  ...
  function handleHelpPagePointerDown(delta, button) {
    if (button.disabled) { return; }
    // pointerdown is the earliest reliable user-gesture boundary for audio,
    // including direct file:// launches in Chromium-based browsers.
    helpPointerHandled = true;
    setHelpPage(helpPageIndex + delta, true);
  }

  helpPreviousButton.addEventListener("pointerdown", function () {
    handleHelpPagePointerDown(-1, helpPreviousButton);
  });

  helpNextButton.addEventListener("pointerdown", function () {
    handleHelpPagePointerDown(1, helpNextButton);
  });

  helpPreviousButton.addEventListener("click", function () {
    if (helpPointerHandled) {
      helpPointerHandled = false;
      return;
    }
    TankGame.Audio.initialize();
