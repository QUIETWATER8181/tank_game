(function () {
  ...
  Game.prototype.detonateBossBomb = function (bullet) {
    ...
    var particleScale = bullet.fixedTurretMortar || bullet.turretWeapon === "bomb" ? 1.45 : 1;
  ...
  Game.prototype.spawnBossMeteors = function (count, targets) {
    ...
    for (var index = 0; index < count; index += 1) {
      var target = targetList[index] || {
        x: Math.max(90, Math.min(Config.worldWidth - 90, player.x + (Math.random() - 0.5) * 980)),
        y: Math.max(90, Math.min(Config.worldHeight - 90, player.y + (Math.random() - 0.5) * 720))
      };
      var startX = Math.max(0, Math.min(Config.worldWidth, target.x + (Math.random() - 0.5) * 260));
      var startY = Math.max(0, target.y - 620 - Math.random() * 180);
      var meteor = {
        x: startX,
        y: startY,
        startX: startX,
        ...
        targetX: target.x,
        targetY: target.y,
        timer: Config.bossMeteor.duration * (0.72 + Math.random() * 0.28),
        duration: Config.bossMeteor.duration,
        progress: 0,
        radius: Config.bossMeteor.radius,
        damage: Config.bossMeteor.damage,
        rotation: Math.random() * Math.PI * 2,
        spin: (Math.random() - 0.5) * 8,
        alive: true
      };
      meteor.duration = meteor.timer;
  ...
  Game.prototype.impactBossMeteor = function (meteor) {
    ...
    obstacles.forEach(function (obstacle) {
      ...
      if (obstacle.kind === "W") {
        TankGame.Map.dryWaterCell(self.worldMap, obstacle);
      } else if (["#", "B", "I"].indexOf(obstacle.kind) !== -1) {
        TankGame.Map.removeObstacle(self.worldMap, obstacle);
      }
    });
  ...
  Game.prototype.drawBossBattleEffects = function (context) {
    ...
    this.bossMeteors.forEach(function (meteor) {
      ...
      var trailLength = 52 + meteor.progress * 48;
      context.save();
